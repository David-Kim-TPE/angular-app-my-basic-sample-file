...
describe('Starting tests for angular-app-file', function() {
  ...
  it('website title should be angular-app-file', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('angular-app-file');
    })
  });

  it('network-name should be my-basic-sample@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('my-basic-sample@0.0.1.bna');
    });
  });

  it('navbar-brand should be angular-app-file',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('angular-app-file');
    });
  });
